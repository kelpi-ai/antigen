...
describe("createRun", () => {
  it("creates a run directory and metadata file path", async () => {
    ...
    expect(run.screenshotsDir).toBe(join(run.runDir, "screenshots"));
    ...
    expect((await stat(run.screenshotsDir)).isDirectory()).toBe(true);
    ...
    expect(metadata.screenshotsDir).toBe(run.screenshotsDir);
    expect(metadata.videoPath).toBe(run.videoPath);
    expect(metadata.recording).toEqual({
      status: "pending",
